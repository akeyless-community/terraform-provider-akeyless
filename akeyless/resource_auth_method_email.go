...
func resourceAuthMethodEmail() *schema.Resource {
	return &schema.Resource{
		Description: "Email Auth Method Resource",
		Create:      resourceAuthMethodEmailCreate,
		Read:        resourceAuthMethodEmailRead,
		Update:      resourceAuthMethodEmailUpdate,
		Delete:      resourceAuthMethodEmailDelete,
		Importer: &schema.ResourceImporter{
			State: resourceAuthMethodEmailImport,
		},
...
func resourceAuthMethodEmailCreate(d *schema.ResourceData, m interface{}) error {
	...
	body := akeyless_api.AuthMethodCreateEmail{
		Name:  name,
		Email: email,
		Token: &token,
	}
	...
	rOut, resp, err := client.AuthMethodCreateEmail(ctx).Body(body).Execute()
	if err != nil {
...
func resourceAuthMethodEmailRead(d *schema.ResourceData, m interface{}) error {
	provider := m.(*providerMeta)
	client := *provider.client
	token := *provider.token
	...
	if rOut.AccessInfo.EmailPassAccessRules != nil && rOut.AccessInfo.EmailPassAccessRules.Email != nil {
		err = d.Set("email", *rOut.AccessInfo.EmailPassAccessRules.Email)
		if err != nil {
			return err
		}
	}
...
func resourceAuthMethodEmailUpdate(d *schema.ResourceData, m interface{}) error {
	provider := m.(*providerMeta)
	client := *provider.client
	token := *provider.token
	...
	body := akeyless_api.AuthMethodUpdateEmail{
		Name:  name,
		Token: &token,
	}
	common.GetAkeylessPtr(&body.AccessExpires, accessExpires)
	...
	_, resp, err := client.AuthMethodUpdateEmail(ctx).Body(body).Execute()
	if err != nil {
...
func resourceAuthMethodEmailDelete(d *schema.ResourceData, m interface{}) error {
	provider := m.(*providerMeta)
	client := *provider.client
	token := *provider.token
...
func resourceAuthMethodEmailImport(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {
	id := d.Id()

	err := resourceAuthMethodEmailRead(d, m)
	if err != nil {
		return nil, err
	}
