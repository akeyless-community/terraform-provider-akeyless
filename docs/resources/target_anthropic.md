---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "akeyless_target_anthropic Resource - terraform-provider-akeyless"
subcategory: ""
description: |-
  Anthropic Target resource
---

# akeyless_target_anthropic (Resource)

Anthropic Target resource



<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `name` (String) Target name

### Optional

- `anthropic_url` (String) Base URL of the Anthropic API
- `api_key` (String, Sensitive) API key for Anthropic
- `delete_protection` (String) Protection from accidental deletion of this object [true/false]
- `description` (String) Description of the object
- `keep_prev_version` (String) Whether to keep previous version [true/false]. If not set, use default according to account settings
- `key` (String) The name of a key that used to encrypt the target secret value (if empty, the account default protectionKey key will be used)
- `max_versions` (String) Set the maximum number of versions, limited by the account settings defaults.

### Read-Only

- `id` (String) The ID of this resource.


