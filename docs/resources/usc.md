...
- `gcp_folder_id` (String) GCP Folder ID (Relevant only for GCP targets with folder scope)
- `gcp_organization_id` (String) GCP Organization ID (Relevant only for GCP targets with folder or organization scope)
- `gcp_project_id` (String) GCP Project ID (Relevant only for GCP targets)
- `gcp_scope` (String) GCP USC scope [project/folder/organization]. Defaults to project when empty
